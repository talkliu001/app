...
本应用采用**本地优先**设计：检测与阶段判定均在设备本地完成，不依赖联网；**不存储**检测题目与您的选项答案，答题内容仅用于当次本地计算；若触发危机熔断，仅保存触发时间与次数，**绝不**保存选项原文或您输入的任何文字。数据仅保存在您设备本地，我们不会上传至服务器或用于广告、用户画像。

完整说明见：**[隐私政策](docs/privacy.html)**（仓库内 [docs/privacy.html](docs/privacy.html)）。若使用 GitHub Pages 发布，可对外使用链接：`https://<你的用户名>.github.io/app/docs/privacy.html`，供应用市场上架时填写隐私政策地址。
